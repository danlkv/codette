...
3. The server renders a consent page with a single button: **Try without registration for N days**. (Sign-in-with-Google and similar IdP options are not implemented in v1.) Submitting POSTs `/oauth/interaction/<uid>/trial` with the OAuth state forwarded.
