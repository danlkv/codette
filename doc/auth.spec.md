...
5. The `/auth/success` page extracts the local port from the `state` parameter and attempts `fetch('http://localhost:<port>/callback?code=…', { mode: 'no-cors' })`. If the local CLI listener responds, the page updates to "Authenticated." If the fetch fails (CLI is running on a remote machine), the page falls back to displaying the code with a copy button so the user can paste it into the CLI prompt. The auth code therefore arrives at the CLI via the success page's JS fetch, not via a browser redirect to a localhost URI.
