...
| Command | SDK method |
|---------|-----------|
| `/model <alias\|id>` | `setModel()`; idle session → applied via resume; before the session exists → queued and applied at session start. Outcome is host-confirmed via `agent_ctl_result` — the client never displays success locally. The context bar shows the current model alongside token usage |
