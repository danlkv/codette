...
// Persists to $OAUTH_DATA_DIR/trial-claims.json.
...
// (allowed), false if blocked. Prevents TOCTOU races from concurrent requests.
...
// Revoke the most recent claim for an IP. Used to roll back a claim when the
// downstream grant save or interactionFinished fails after claimIfAllowed
// already recorded the timestamp.
...
// Test seam — file-backed implementation has no in-memory state to clear, but
// the symbol is exported for symmetry with cache-based variants.
export function _resetForTest() { /* no-op */ }
