...
import { verifyHandshakeProof } from './ws-auth.js';

const AUD = 'https://srv.example/host';
...
const lookup = (jkt, jwk, username) => (fp) => (fp === jkt ? { username, jwk } : null);

test('rejects handshake proof missing exp', async () => {
  ...
  const jwt = await new SignJWT({})
    ...
    .sign(key);
  assert.equal(await verifyHandshakeProof({
    proofJwt: jwt, lookupByPubkey: lookup(jkt, jwk, 'alice'),
    expectedAud: AUD, jtiCache: freshJtiCache(),
  }), null);
});
...
test('rejects iat well in the future (2 minutes)', async () => {
  ...
  const jwt = await new SignJWT({})
    .setProtectedHeader({ alg: 'ES256' })
    .setIssuer(jkt).setAudience(AUD)
    .setIssuedAt(future).setExpirationTime(future + 60).setJti(randomBytes(8).toString('hex'))
    .sign(key);
  assert.equal(await verifyHandshakeProof({
    proofJwt: jwt, lookupByPubkey: lookup(jkt, jwk, 'alice'),
    expectedAud: AUD, jtiCache: freshJtiCache(),
